...
export function TasksPage() {
  ...
  const [artifacts, setArtifacts] = useState<Record<string, api.ArtifactRecord[]>>({});
  const [previewArtifact, setPreviewArtifact] = useState<Record<string, api.ArtifactRecord | null>>({});
  ...
  const loadArtifacts = async (run: api.TaskRun) => {
    try {
      const items = await api.listArtifacts(run.id);
      setArtifacts((current) => ({ ...current, [run.id]: items }));
    } catch {
      // 产物是辅助视图，失败不阻塞任务列表
    }
  };
  ...
  useEffect(() => {
    ...
    Object.values(runs).forEach((run) => { void loadArtifacts(run); });
  }, [runs]);
  useEffect(() => {
  ...
  return <section className="page tasks-page" aria-labelledby="tasks-title">
    ...
    {!error && !loading && groups.map((group) => group.items.length > 0 && <section className={`task-group ${group.key}`} key={group.key}><header><span>{group.label}</span><b>{group.items.length}</b></header>{group.items.map((task) => {
      const done = task.status === "done"; const run = runs[task.id]; const items = history[task.id] || []; const rec = run ? recovery[run.id] : undefined; const retryNode = (run.nodes || []).find((n) => (n.status === "running" || n.status === "ready") && n.tool_ref);
