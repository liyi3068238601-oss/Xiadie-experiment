...
- [x] 任务详情、实时状态、等待原因、下一动作和失败证据完整可读。
- [x] 取消竞态、进程崩溃、数据库忙、重复请求和陈旧 revision 故障注入。
- [x] 任务事件与诊断日志、ToolRun、未来 Artifact 全链路关联。
- [x] Windows 启动器与崩溃恢复测试（进程级 E2E；打包/安装链路由 CYR.9 承接）。
- [x] 多模型计划质量固定集；模型未验证仍可使用，不作为运行许可证。
