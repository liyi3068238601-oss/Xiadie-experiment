...
### CYR.1S：启动安全稳定化（2026-08-02）

- [x] PR #1 可观测性基线和 PR #2 CYR.1 按堆叠顺序合入 `main`。
- [x] 启动自检逐项验证 manifest、资源路径、SHA-256 和 token 门，并写入 body-free 诊断日志。
- [x] 增加极短、代码内置且无外部依赖的 `persona-emergency-v1`。
- [x] 故障注入覆盖文件缺失、hash 不匹配、非法路径和 v2.3/v2.2 同时失效。
- [x] 设置页分开展示 Persona 资源、模型质量和运行能力状态。
- [x] 未验证模型继续加载 v2.3，Persona 不控制 temperature 或其他采样参数。
- [x] README、长期退场台账、定向测试与全量门禁同步更新。
- [-] 启动器真实使用观察需要时间，按用户决定延后，不作为本轮阻塞项。

CYR.1S 只增强启动与诊断安全，不删除 v2.2，也不把模型质量记录重新变成运行许可证。退出门是资源损坏可以被准确定位、聊天仍有安全保底、设置页不混淆“未验证”和“不可运行”，且日志不包含 Persona 正文。

工程验证记录：后端全量 `2518 passed`，相关定向 `105 passed`，前端 `81 passed`，Vite 生产构建与 Python `compileall` 通过；正常资源自检为 v2.3 `1442 tokens`、v2.2 `1400 tokens`、emergency `195 tokens`。真实使用体验结论未填写，等待后续观察。
